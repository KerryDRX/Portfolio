...
class DenseNormalGamma(torch.nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.dense = nn.Linear(in_features, out_features * 3)
    ...
    def forward(self, x):
        output = self.dense(x)
        logv, logalpha, logbeta = torch.split(output, self.out_features, -1)
...
class DeterministicEncoder(nn.Module):
	def __init__(self, num_latent, img_dims, loader_dir):
		...
		self.pca_pred = nn.Sequential(OrderedDict([
			('linear', nn.Linear(hidden2, self.num_latent))
		]))
...
class DeterministicLinearDecoder(nn.Module):
	def __init__(self, num_latent, num_corr):
		...
		self.fc_fine2 = DenseNormalGamma(self.num_latent, self.numL*3)

	def forward(self, pca_load_unwhiten):
		mu = self.fc_fine(pca_load_unwhiten)
		v, alpha, beta = self.fc_fine2(pca_load_unwhiten)
		corr_out = mu.reshape(-1, self.numL, 3)
		return corr_out, [mu, v, alpha, beta]
...
class DeepSSMNet(nn.Module):
	...
	def forward(self, x):
		...
		corr_out, [mu, v, alpha, beta] = self.decoder(pca_load_unwhiten)
		return [pca_load, corr_out, [mu, v, alpha, beta]]