...
dataset_directory = f'../dataset/{city}/Ventricles_64_3_partial/'
deepssm_directory = f'{dataset_directory}DeepSSM_500_256par_13reg/'
...
meshes = [image.toMesh(isovalue=0.5).smooth(iterations=500).fillHoles() for image in images]
...
train_val_test_ratio = (0.8, 0.2, 0)
...
subjects = []
for train_mesh_path, train_transform in tqdm(
    zip(all_mesh_paths['train'], train_transforms), total=train_size, desc='Creating subjects'
):
    subject = sw.Subject()
...
print('Optimizing training particles ...')
